    ...
    function parseOCR(text) {
      ...
      // A posição é o campo mais confiável da folha e tem formato fixo.
      // Não exigimos "HRW", pois em fontes pequenas o OCR pode perder essa
      // palavra mesmo quando lê perfeitamente a posição.
      const locRegex = /\b([0-9OILSZ]{1,3})\s*[-/]\s*([0-9OILSZ]{1,3})\s*[-/]\s*([0-9OILSZ]{1,3})\b/g;
      const rawLocMatches = [...source.matchAll(locRegex)];
      if (!rawLocMatches.length) return [];

      const fixDigit = value => String(value || '')
        .replace(/O/g, '0')
        .replace(/[IL]/g, '1')
        .replace(/S/g, '5')
        .replace(/Z/g, '2');

      const locMatches = rawLocMatches.map(m => ({
        ...m,
        street: fixDigit(m[1]),
        block: fixDigit(m[2]),
        level: fixDigit(m[3])
      ...
      const codeRegex = /\bF\s*\d{6,}\b/g;
      const loteRegex = /\bH\s*\d[A-Z0-9]{3,}\b/g;
      const itemRegex = /\b\d{4}\b/g;
      const codes = [...source.matchAll(codeRegex)];
      const lotes = [...source.matchAll(loteRegex)];

      const out = [];
      const seen = new Set();

      locMatches.forEach((m, i) => {
        const location = `${m.street}-${m.block}-${m.level}`;
        const key = normalizeLocationKey(location);
        if (!key || seen.has(key)) return;

        const prevBoundary = i > 0 ? locMatches[i - 1].index : 0;
        const nextBoundary = i + 1 < locMatches.length ? locMatches[i + 1].index : source.length;

        // O número do item só é aceito quando está realmente próximo da
        // posição. Assim, se o OCR perder o 0024, não roubamos o 0025.
        const beforeTail = source.slice(Math.max(prevBoundary, m.index - 90), m.index);
        const itemCandidates = [...beforeTail.matchAll(itemRegex)].map(x => x[0]);
        const no = itemCandidates.length ? itemCandidates[itemCandidates.length - 1] : '';

        const block = source.slice(m.index, nextBoundary);
        const blockCode = block.match(codeRegex);
        const blockLote = block.match(loteRegex);

        const nearestToken = (matches, pos, maxDistance = 180) => {
          let best = '';
          let bestDist = Infinity;
          for (const x of matches) {
            const d = Math.abs(x.index - pos);
        ...
        const code = (blockCode?.[0] || nearestToken(codes, m.index)).replace(/\s+/g, '');
        const batch = (blockLote?.[0] || nearestToken(lotes, m.index)).replace(/\s+/g, '');

        out.push({
          no: no || String(out.length + 1).padStart(4, '0'),
          location,
          code,
          batch,
          product: ''
    ...
    function preprocessImageForOCR(dataUrl) {
      return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
          try {
            // Não binarizar agressivamente: nas suas folhas os caracteres são
            // pequenos e finos, e Otsu/sharpen forte pode apagar traços.
            const maxDim = Math.max(img.width, img.height);
            const targetMax = 3200;
            let scale = targetMax / maxDim;
            scale = Math.min(Math.max(scale, 1), 2.0);
            ...
            for (let i = 0; i < n; i++) {
              let v = 128 + (gray[i] - (min + max) / 2) * (180 / range);
    ...
    function mergeExtractedRows(preferred, fallback) {
      ...
      [...(preferred || []), ...(fallback || [])].forEach(x => {
        const key = normalizeLocationKey(x.location) || (x.code || x.batch || x.no);
        if (!key) return;
        ...
        map.set(key, {
          ...old,
          no: old.no && !/^0+$/.test(old.no) ? old.no : (x.no || old.no),
          code: old.code || x.code || '',
          batch: old.batch || x.batch || '',
          product: old.product || x.product || ''
        });
      });
      return [...map.values()];
    }
    ...
    async function recognizeWithPreprocessing(img, onProgress) {
      ...
      // PRIMEIRA PASSADA: imagem original + PSM 11. Esta é a mais importante
      // para as suas folhas porque o layout tem várias colunas e grandes
      // espaços entre os registros.
      await worker.setParameters({
        tessedit_pageseg_mode: '11',
        preserve_interword_spaces: '1'
      });
      const original = await worker.recognize(img.data);
      const rowsOriginal = parseOCR(original.data.text || '');

      // SEGUNDA PASSADA: imagem ampliada com contraste suave. Não usamos
      // binarização pesada porque ela piorou as fontes pequenas nas suas fotos.
      const processedDataUrl = await preprocessImageForOCR(img.data);
      const processed = await worker.recognize(processedDataUrl);
      const rowsProcessed = parseOCR(processed.data.text || '');

      let merged = mergeExtractedRows(rowsOriginal, rowsProcessed);

      // TERCEIRA PASSADA somente quando ainda há poucos resultados.
      if (merged.length < Math.max(rowsOriginal.length, rowsProcessed.length, 2)) {
        await worker.setParameters({
          tessedit_pageseg_mode: '6',
          preserve_interword_spaces: '1'
        });
        const psm6 = await worker.recognize(processedDataUrl);
        merged = mergeExtractedRows(merged, parseOCR(psm6.data.text || ''));
      }
    ...
    function getSectorFromLocation(location) {
      const match = String(location || '').match(/^\s*(\d+)\s*[-–/]/);
      if (!match) return null;

      const street = Number.parseInt(match[1], 10);
